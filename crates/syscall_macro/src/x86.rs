...
pub fn gen_syscalls(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        #[link_section = ".data"]
        #[no_mangle]
        static SYSCALLS: [fn(&crate::platform::x86::cpu::InterruptFrame) -> usize; #num_syscalls as usize] = [
            #(#syscall_entries),*
        ];
    };
...
pub fn syscall(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    for (i, arg) in input_fn.sig.inputs.into_iter().enumerate() {
        if let syn::FnArg::Typed(PatType { pat, ty, .. }) = arg {
            if let syn::Pat::Ident(ref ident) = *pat {
                ...
                let exp = quote! {
                    let #ident: #ty = crate::platform::x86::process::task::Task::copy_stack_item(crate::platform::x86::process::task::CurrentTask::get(), #idx);
                };
                decl.push(exp);
            }
        }
    ...
    let expanded = quote! {
        fn #uniform_name(frame: &crate::platform::x86::cpu::InterruptFrame) -> usize {
            #(#decl)*
